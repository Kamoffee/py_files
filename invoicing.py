...
def next_invoice_number(invoice_number: str) -> str:
    ...
    if year == current_year:
        # next_invoice_number = f'{current_year}-{NNNN + 1:04d}'
        number += 1
    else:
        # next_invoice_number = f'{current_year}-1'
        invoice_year = year
        number = 1
    next_invoice_number = f'{current_year}-{number:04d}'
...
def record_invoice(invoice_file: TextIO,
    ...
    :param last_line_ptr: The position of the start of the last line in the file.
        This will be obtained by previous call to 'record voice'.
...
with open(data_file, 'r+') as invoices:
    ...
    last_line = record_invoice(invoices, 'Squirrel Storage', 320.5, last_line)
